 ...
 * filed.
 */
#define BOOT_GET_SWAP_TYPE(swap_info)                \
    ({                                               \
        uint8_t data = (swap_info) & 0x0FU;          \
        boot_swap_type_t res = BOOT_SWAP_TYPE_PANIC; \
                                                     \
        if (IS_VALID_SWAP_TYPE(data)) {              \
            res = (boot_swap_type_t)data;            \
        }                                            \
                                                     \
        res;                                         \
    })
...
#define BOOT_SET_SWAP_INFO(swap_info, image, type)      \
    {                                                   \
        assert((image) < 0x0FU);                        \
        assert(IS_VALID_SWAP_TYPE(type));               \
        (swap_info) = (image) << 4U | (uint8_t)(type);  \
    }
